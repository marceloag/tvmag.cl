...
function ClientPage({ initialCanales, defaultCanal }) {
  ...
  useEffect(() => {
    if (canal && canales.length > 0) {
      const selectedChannel = canales.find(
        (c) => c.canal.toLowerCase() === canal.toLowerCase()
      );
      if (selectedChannel) {
        setIsPlaying(false);
        setStream(selectedChannel);
        setIsPlaying(true);
      }
    }
  }, [canal, canales]);
